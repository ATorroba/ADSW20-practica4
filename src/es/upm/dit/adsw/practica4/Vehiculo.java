...
/**
 * @author Álvaro Torroba de Linos
 * @author Alejandro Mariñas Cuesta
 *
 */
public class Vehiculo {
	...
	/**
	 * Estima la posicion en la que nos encontraremos en el instante t. pos0 pasara
	 * a ser la actual y pos la estimacion
	 * 
	...
	/**
	 * Estima la posicion en la que nos encontraremos en el instante t, y si en ese
	 * instante estaremos en una posicion proxima a pos
	 * 
	...
	/**
	 * Calcula el tiempo que queda para que impactemos con otro vehiculo. Si este
	 * vehiculo y v2 va a impactar devuelve el tiempo que queda en funcion de la
	 * velocidad y la trayectoria de los vehiculos Si este vehiculo y el otro se
	 * estan separando devuelve un negativo que es una estimacion de a que velocidad
	 * se separan Si los dos vehiculos se encuentran casi pegados devuelve 0
	 * 
	...
	/**
	 * Devuelve true si el vehiculo v2 no guarda una distancia minima respeto de
	 * este vehiculo o si no se guarda la distancia de seguridad en funcion de la
	 * velocidad
	 * 
	...
	public boolean isPeligroso(Vehiculo v2) {
		return (this.getPos().distancia(v2.getPos()) < MINIMAL_DISTANCE
				|| Math.abs(this.impacto(v2)) * this.getVelocidad().modulo() < SAFETY_DISTANCE);
	}
	 ...
	 * @throws IllegalArgumentException el vehiculo del navegador debe ser este
	 *                                  vehiculo
	 */
	 ...
	 * 
	 * @return los detectores OJO EN EL CASO PREVIO SÖLO SE TENIA UN DETECTOR.
	 *         ELEGIR UNO Devuelve el detector del vehiculo
	 * @return el detector
	 */
	...
	/**
	 * Servicio para dirigir el movimento del vehiculo desde su posición actual a la
	 * posicion/vector de destino
	 * 
	 ...
	 * @throws IllegalArgumentException si el destino no esta en el mapa del
	 *                                  navegador
	 */
	public List<Vector> irA(Vector destino) {
		...
		while (!navegador.finMovimiento()) {
			...
			// TODO Utilizar el ControlEntreda ce para evitar que dos vehiculos estén al
			// mismo tiempo en un tramo
			ce.entrarEnTramo();
			
			getNavegador().getMapa().mueve(this, c);
			
			// TODO Hemos terminado de hacer el tramo, debemos actualizar ce dicendo que
			// hemos salido
			ce.salirDeTramo();
			...
			LOGGER.log(Level.INFO, Vehiculo.this.id + " llega a " + Vehiculo.this.getPos() + " a las " + this.getT(),
					params2);
		}
	...
	/**
	 * Crea una hebra independiente que representa el movimiento del vehiculo. El
	 * vehiculo estara iendo y viniendo desde su posicion actual hasta el destino,
	 * el numero de vueltas indicado, utilizando el metodo irA para hacer cada
	 * trayecto
	 * 
	 * @param destino destino a que vamos y volvemos desde la posicion actual
	 * @param vueltas numero de veces que vamos/volvemos. Por ejemplo un 2 nos dice
	 *                que vamos y despues volvemos y la hebra termina
	...
	public Thread arrancaVehiculo(Vector destino, int vueltas) throws Exception {
		if (getNavegador() == null)
			throw new Exception("solo podemos arrancar con el navegador inicializado");
			...
			public void run() {
				...
				try {
					for (int i = 0; i < vueltas; i++) {
						...
						// LOGGER.info("Soy "+Vehiculo.this.id+" y he llegado a
						// "+Vehiculo.this.getPos());
						Object[] params = { Vehiculo.this.id, Vehiculo.this.getPos(), Vehiculo.this.getT() };
						LOGGER.log(Level.INFO, "Soy " + Vehiculo.this.id + " y he llegado a " + Vehiculo.this.getPos(),
								params);
						Vector tmp = hasta;
						hasta = desde;
						desde = tmp;
	...
	public static void main(String[] arg) {
		/*
		 * String f="grafo.txt"; if (arg.length > 0) f=arg[0]; GrafoPosiciones gp = new
		 * GrafoPosiciones(f); Vehiculo v=new Vehiculo("id000",new
		 * Vector(-10.0,0.0),0.0,new Vector(-9.0,0.0),1.0); // Vector hasta = new
		 * Vector(0.0,0.0); Vector hasta = new Vector(0.0,30.0); v.setNavegador(new
		 * NavegadorImpl(v,gp)); try { v.navegador.setSecuenciaMovimientos(hasta); while
		 * (!v.navegador.finMovimiento()) { Tramo c = v.navegador.siguienteMovimiento();
		 * System.out.println(c.desde()); System.out.println(c.hasta());
		 * System.out.println(c.tiempo()); } } catch(NoSuchElementException e) {
		...
		/*
		 * String f="grafo.txt"; if (arg.length > 0) f=arg[0]; GrafoPosiciones gp = new
		 * GrafoPosiciones(f); gp.dibuja(); Vehiculo v=new Vehiculo("id000",new
		 * Vector(-10.0,0.0),0.0,new Vector(0.0,0.0),1.0); // Vector hasta = new
		 * Vector(0.0,0.0); Vector desde = new Vector(0.0,0.0); Vector hasta = new
		 * Vector(0.0,30.0); v.setNavegador(new NavegadorImpl(v,gp)); try { for (int
		 * i=0; i < 10; i++) { v.irA(hasta); LOGGER.info("Nos hemos ido a "+v.getPos());
		 * Vector tmp=hasta; hasta=desde; desde=tmp; } } catch(NoSuchElementException e)
